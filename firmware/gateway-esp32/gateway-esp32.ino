...
void registerGateway() {
  ...
  String checkUrl = String(SUPABASE_URL)
    ...
    + "&name=eq." + GATEWAY_NAME + "&select=id&limit=1";
  ...
  if (existing == "[]" || existing.isEmpty()) {
    ...
    String insertBody = "{\"church_id\":\"" + String(CHURCH_ID)
      + "\",\"name\":\"" + GATEWAY_NAME + "\",\"last_seen\":\"" + now + "\"}";
    // Usar Prefer: return=representation para obter o UUID gerado
    ...
    if (code == 201) {
      ...
      Serial.printf("[HTTP] Gateway inserido. Resposta: %s\n", resp.c_str());
      Serial.println("[BOOT] >>> Copie o 'id' acima para #define GATEWAY_ID <<<");
    } else {
      Serial.printf("[HTTP] Falha ao inserir gateway: %d\n", code);
    }
    http.end();
  } else {
    String patchUrl = String(SUPABASE_URL)
      + "/rest/v1/gateways?church_id=eq." + CHURCH_ID + "&name=eq." + GATEWAY_NAME;
    String body = "{\"last_seen\":\"" + now + "\"}";
    httpPatch(patchUrl.c_str(), body.c_str());
    Serial.printf("[HTTP] Gateway '%s' atualizado\n", GATEWAY_NAME);
  }
}
